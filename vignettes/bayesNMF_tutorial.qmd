...
#### `label_switching.png`
Diagnostic plot of label switching, showing the closest reference match for each signature for each posterior sample.
Only created if learning rank and `save_all_samples = TRUE`. If label switching occurs, the color of a latent factor will switch part way through the chain. 
